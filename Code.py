# -*- coding: utf-8 -*-
"""PySpark_Assignment_Sep23 (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1mHe1ovGU3OXp4FUYvdmGAJf5DLydYpXT

# ADS2 - Assignment 1 - Data Handling and Processing with PySpark

**STUDENT NAME -** **Bubly Babu**
...
tvDF = (tvDF.withColumn('season_title_null', F.when(F.isnull(tvDF['season_title']), tvDF['show_title']).otherwise(tvDF['season_title']))\
          .drop('season_title')\
          .withColumnRenamed('season_title_null', 'season_title'))
tvDF.show()
...
STDF = (tvDF.where(
                  (tvDF['show_title'] == 'Stranger Things') & \
...
Top25 = (tvDF.filter(tvDF['country_name'] == 'United Kingdom')\
             ...
             .sort('total_weeks', ascending = False)\
             .limit(25)
         )
...
week_per_season = (netflixDF.filter(netflixDF['show_title'] == 'Young Sheldon')\
                                 .groupBy('season_title', 'country_name')\
                                 .agg(F.max('cumulative_weeks_in_top_10').alias('max_week'))\
                                 .sort('max_week', ascending = False))
top_countries_season = (week_per_season.groupBy('season_title')\
                                       .agg(
                                            F.first('country_name').alias('country_most_spent'),
                                            F.max('max_week').alias('max_week')))
top_countries_season.show(truncate = False)
...
film_data = (filmsDF.groupBy('country_name','show_title')\
                    .agg(
                        F.max('cumulative_weeks_in_top_10').alias('max_week'))\
                    .sort('max_week', ascending = False))

most_spent_film = (film_data.groupBy('country_name')\
                           .agg(
                              F.first('show_title').alias('most_spent_film'),
                              F.max('max_week').alias('max_week')
                               )\
                    .orderBy('country_name'))

most_spent_film.show(truncate = False)
...
film_at_1DF = (filmsDF.withColumn('weeks_at_1',
                    F.when(filmsDF['weekly_rank'] == 1, filmsDF['cumulative_weeks_in_top_10'])
                    .otherwise(0)\
                    .alias('weeks_at_1'))
                    )
film_at_1DF.show(truncate = False)
...
### country.
top_filmDF = (film_at_1DF.groupBy('country_name', 'show_title')
                           .agg(F.count('weeks_at_1').alias('count_weeks_at_1'))
                           .sort('count_weeks_at_1', ascending = False)
             )
top_filmDF.show(truncate = False)
...
resultDF = film_at_1DF.groupBy('country_name','show_title')\
                    .agg(F.sum('weeks_at_1').alias('count_weeks_at_1'))\
                    .orderBy('count_weeks_at_1', ascending=False)
